...
RSpec.describe Parse do
  ...
  context 'find_method' do
    it 'find the correct ast' do
      expect(p.find_method(:find_organiztion).loc.keyword.line).to eq(44)
    end
    it 'return nil when not found' do
      expect(p.find_method(:no_method)).to be(nil)
    end
  end

  it 'find_relative_key_on_ast' do
    ast = p.find_method(:check_business_info_type)
    expect(p.find_relative_key_on_ast(ast)).to eq([{ :line => 50, :key => '.plase_fill_in_info_for_invoice_generation', :file => FILE, :method => :check_business_info_type }])
  end
end
